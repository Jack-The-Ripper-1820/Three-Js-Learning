...
// #region PlaneShadowFolder
const planeShadowFolder = gui.addFolder('PlaneShadow')
planeShadowFolder.add(plane, 'castShadow')
planeShadowFolder.add(plane, 'receiveShadow')
planeShadowFolder.close()

const meshShadowFolder = gui.addFolder('MeshShadow');
const meshTypes = ['MeshBasic', 'MeshNormal', 'MeshPhong', 'MeshStandard'];

meshTypes.forEach((type, i) => {
  const folder = meshShadowFolder.addFolder(type);
  folder.add(meshes[i], 'castShadow');
  folder.add(meshes[i], 'receiveShadow');
});

meshShadowFolder.close();


// #endregion

// #region AmbientLight
const ambientLight = new THREE.AmbientLight(data.lightColor, 0.5);
...
const hemisphereLight = new THREE.HemisphereLight(0x00aaff, 0xffaa00, 0.6); // Sky color, Ground color, Intensity
...
hemisphereLightFolder.add(hemisphereLight, 'intensity', 0, 2, 0.01);
...
const rectAreaLight = new THREE.RectAreaLight(0xffffff, 1, 4, 2); // Color, Intensity, Width, Height
...
rectAreaLightFolder.addColor(data, 'lightColor').onChange(() => {
  rectAreaLight.color.set(data.lightColor);
});
rectAreaLightFolder.add(rectAreaLight, 'intensity', 0, 10, 0.1);
rectAreaLightFolder.add(rectAreaLight, 'width', 0.1, 10, 0.1);
rectAreaLightFolder.add(rectAreaLight, 'height', 0.1, 10, 0.1);
