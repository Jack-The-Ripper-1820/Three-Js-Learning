...
new OBJLoader().loadAsync('models/suzanne.obj').then((object) => {
  ...
  const vertices = new Float32Array(suzanneMesh.geometry.attributes.position.array)
  // let indices = new Uint32Array((suzanneMesh.geometry.index as THREE.BufferAttribute).array)
  const suzanneShape = (RAPIER.ColliderDesc.convexHull(vertices) as RAPIER.ColliderDesc).setMass(1).setRestitution(0.5)
